...
class PassengerServiceImplTest {
    ...
    @InjectMockKs
    var passengerService: PassengerServiceImpl? = null
    ...
    fun findById_shouldReturnExpectedResponse() {
        ...
        val actual: PassengerResponse = passengerService?.findById(TestData.PASSENGER_ID) ?: throw AssertionError()
    ...
    fun findAll_shouldReturnExpectedPage() {
        ...
        val actual: Page<PassengerResponse> = passengerService?.findAll(pageable) ?: throw AssertionError()
    ...
    fun create_shouldReturnCreatedObject() {
        val passenger: Passenger = TestData.defaultPassenger()
        passenger.id = null
        val expected = TestData.defaultPassengerResponse().copy(id = 0)

        every { passengerRepository.findByEmail(TestData.EMAIL) } returns Optional.empty()
        every { passengerRepository.findByPhoneNumber(TestData.PHONE_NUMBER) } returns Optional.empty()
        every { passengerRepository.save(passenger) } returns passenger

        val actual: PassengerResponse =
            passengerService?.create(TestData.defaultPassengerCreate()) ?: throw AssertionError()
